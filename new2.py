        ...
        **Assumptions:** 
        ...
        if st.button("Predict Dropout Risks"):
            with st.spinner("Training model and predicting..."):
                ...
                # Convert predictions to Series and count
                counts = pd.Series(predictions).value_counts()

                # Get count of high-risk students (assuming label = 1 means "high risk")
                high_risk_count = counts.get(1, 0)   # default to 0 if no "1"s

                # Show in Streamlit metric
                st.metric("High Risk Students (Prob > 0.5)", high_risk_count)
